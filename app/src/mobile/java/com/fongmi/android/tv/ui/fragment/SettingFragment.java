    ...
    private void onVersion(View view) {
        ...
        Task.submit(() -> {
            try {
                ...
                String url = baseUrl + "api/update/mobile";
                ...
                App.post(() -> {
                    ...
                    if (cachedVersion == serverVersion && cachedPath != null && new java.io.File(cachedPath).exists()) {
                        UpdateDialog.install(new java.io.File(cachedPath)).show(requireActivity().getSupportFragmentManager(), "update");
                    } else {
                        UpdateDialog.create(serverVersion, apkUrl).show(requireActivity().getSupportFragmentManager(), "update");
                    }
                });
            } catch (Exception e) {
                App.post(() -> {
